...
export const DashboardPage: React.FC = () => {
  ...
  const chartColors = [
    '#6366f1', '#8b5cf6', '#ec4899', '#f43f5e', '#f97316', '#eab308', '#84cc16', '#10b981', '#14b8a6', '#06b6d4',
  ];

  const totalCheckingBalance = checkingAccounts.reduce((sum, acc) => sum + (acc.initialBalance || 0), 0);
  const totalIncome = transactionsForTotals.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amount, 0);
  const totalExpense = transactionsForTotals.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-slate-900">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl flex flex-col items-center">
          ...
          <p className="text-slate-600 dark:text-slate-300 font-medium">Buscando suas finanças...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-900">
        <div className="bg-white dark:bg-slate-800 border border-red-100 dark:border-red-900 rounded-3xl p-8 max-w-md shadow-xl">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Ops, algo deu errado</h3>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-indigo-600 to-indigo-800 dark:from-indigo-600 dark:to-indigo-900 text-white rounded-3xl p-6 shadow-lg shadow-indigo-200/50 dark:shadow-none hover:-translate-y-1 transition-transform duration-300 group">
            ...
            <div className="relative z-10 flex items-center justify-between mb-4">
              <span className="text-sm font-medium text-indigo-100 uppercase tracking-wider">Saldo do Mês</span>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/60 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 group">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Receitas</span>
            ...
            <div className="mt-2 flex items-center gap-1.5 hidden">
              ...
              <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">+12% vs último mês</span>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/60 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 group">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Despesas</span>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/60 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Contas</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-3xl p-6 md:p-8 border border-slate-100 dark:border-slate-700/60 shadow-sm flex flex-col min-h-[420px]">
            <div className="mb-6 flex justify-between items-center">
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Histórico Mensal</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Comparativo de receitas e despesas.</p>
              </div>
            </div>
                    ...
                    y: {
                      ...
                      ticks: {
                        ...
                        callback: function (value) {
                          ...
                          return num >= 1000 ? `R$ ${(num / 1000).toFixed(0)}k` : `R$ ${num}`;
                        }
                      },
                    },
                  },
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/60 shadow-sm flex flex-col min-h-[420px]">
            <div className="mb-6 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Últimas movimentações.</p>
              </div>
            </div>
                ...
                {recentGlobalTransactions.map(tx => (
                  <div key={tx._id} className="flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-105 ${tx.type === 'income'
                      ...
                      <div className="overflow-hidden">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                          {tx.category?.name || 'Sem categoria'} • {format(new Date(tx.date), "dd MMM", { locale: ptBR })}
                        </p>
                      </div>
                    </div>
                    <div className={`text-sm font-bold whitespace-nowrap ${tx.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-white'
                      }`}>
                      {tx.type === 'income' ? '+' : '-'}{CurrencyFormatter.format(tx.amount / 100)}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 md:p-8 border border-slate-100 dark:border-slate-700/60 shadow-sm flex flex-col min-h-[420px]">
            <div className="mb-6">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-1">Por Categoria</h2>
            ...
            {chartData.length > 0 ? (
              <div className="flex-1 flex items-center justify-center relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-6">Total</span>
              ...
              <div className="flex-1 flex flex-col items-center justify-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-medium">Sem dados no período</p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/60 shadow-sm overflow-hidden">
              <CategoryEvolutionChart allTransactions={allTransactions} categories={categories} />
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700/60 shadow-sm overflow-hidden">
              <CategoryComparisonChart allTransactions={allTransactions} categories={categories} />
            </div>
          </div>
