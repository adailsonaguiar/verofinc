...
import { X, Save, DollarSign, Calendar, Tag, CreditCard as CreditCardIcon, CheckCircle2, Clock, AlignLeft, Plus, Loader2 } from 'lucide-react';
...
export const TransactionModal: React.FC<TransactionModalProps> = ({
  ...
  const loadAccounts = async () => {
    try {
      ...
      const allAccounts = [...checking, ...cards];
      setAccounts(allAccounts);
      
      if (allAccounts.length === 0) {
        toast.warning('⚠️ Nenhuma conta cadastrada!', { position: 'top-center' });
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const numericAmount = Number(amount.replace(/[^\d,]/g, '').replace(/\./g, '').replace(',', '.'));
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        {/* Overlay */}
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md transition-opacity animate-in fade-in duration-300" onClick={onClose} />
        ...
        <div className="relative z-10 w-full max-w-xl bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-800 transition-all duration-300 animate-in fade-in zoom-in-95 slide-in-from-bottom-8 duration-500 overflow-hidden">
          ...
          <div className="px-8 py-8 flex items-center justify-between">
            <div className="flex items-center gap-4">
               <div className={`p-3 rounded-2xl ${isEditing ? 'bg-amber-50 dark:bg-amber-500/10 text-amber-600' : 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600'}`}>
                  {isEditing ? <CheckCircle2 className="w-6 h-6" /> : <Plus className="w-6 h-6" strokeWidth={3} />}
               </div>
               <div>
                  <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                    {isEditing ? 'Editar Transação' : 'Nova Transação'}
                  </h2>
                  <p className="text-sm font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">Preencha os detalhes abaixo</p>
               </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="px-8 pb-10 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-[13px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">
                   <CreditCardIcon className="w-4 h-4 text-indigo-500" /> Conta / Cartão
                </label>
                    ...
                    {accounts
                      ...
                      .map(acc => (
                        <option key={acc._id} value={acc._id}>
                          {acc.name} {acc.type === 'credit_card' ? '(Crédito)' : ''}
                        </option>
                      ))}
                  </select>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  {loadingCategories ? (
                    <div className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-950/50 rounded-[1.25rem] border border-slate-100 dark:border-slate-800 text-slate-400 text-sm font-bold animate-pulse">Buscando...</div>
                  ) : (
                    <select
                      value={categoryId}
            ...
            {accounts.find(acc => acc._id === accountId)?.type !== 'credit_card' && (
              <div className="space-y-3 pt-2">
                <label className="flex items-center gap-2 text-[13px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">
                   <Clock className="w-4 h-4 text-indigo-500" /> Status da Transação
                </label>
            ...
            <div className="flex gap-4 pt-8">
              ...
              >
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" strokeWidth={3} />}
